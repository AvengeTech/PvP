...
class TagsComponent extends SaveableComponent{
	...
	public function createTables() : void{
		...
		foreach([
			"CREATE TABLE IF NOT EXISTS tags_data(xuid BIGINT(16) NOT NULL UNIQUE, active VARCHAR(50), tags BLOB)"
	...
	public function finishLoadAsync(?ComponentRequest $request = null) : void{
		...
		if(count($rows) > 0){
			...
			$active = $this->active = PvP::getInstance()->getTags()->getTag($data["active"]);
