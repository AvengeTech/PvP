...
class GuideSelectUi extends SimpleForm{
	...
	public function __construct(Player $player, int $rarity){
		...
		$e = PvP::getInstance()->getEnchantments()->getEffects()->getEffects($rarity, $player->isStaff());
		$e = array_shift($e);

		parent::__construct($e->getRarityName() . " animators", "Select an animator below to get it's description!");

		foreach(PvP::getInstance()->getEnchantments()->getEffects()->getEffects($rarity, $player->isStaff()) as $eff){
			$this->guides[] = $eff;
