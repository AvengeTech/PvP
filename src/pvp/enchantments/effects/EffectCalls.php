<?php namespace pvp\enchantments\effects;
...
use pvp\PvP;
use pvp\enchantments\effects\tasks\{
	FloodAnimationTask,
...
use pvp\enchantments\effects\entities\{
	WitchcraftBat,
	ExplodingChicken,
	CreepySpider,
	CreepySilverfish,
	...
	public function __construct(){
		$this->calls = [
			...
			self::FLOOD => function(Player $killer, $dead){
				...
				PvP::getInstance()->getScheduler()->scheduleDelayedTask($task, 1);
			},
			...
			self::DREAM => function(Player $killer, $dead){
				$entity = new DreamBoat(Location::fromObject($dead->add(0, 0.3), $killer->getWorld()), $dead->add(0, 0.3));
				$entity->spawnToAll();
			},
			...
			self::RIP => function(Player $killer, $dead){
				...
				$entity = new Tombstone(Location::fromObject($pos, $dead->getWorld()), $pos, ($dead instanceof Entity ? $dead->getNameTag() : "rando"));
