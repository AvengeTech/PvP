<?php namespace pvp\enchantments\effects\items;
...
class EffectItem extends Item{
	...
	public function getEffect() : ?EffectClass{
		$effect = ($effs = PvP::getInstance()->getEnchantments()->getEffects())->getEffectById($this->getEffectId());
		if($effect === null){
	...
	public function setup(int $rarity = 0, ?EffectClass $effect = null, int $cost = -1) : void{
		$this->rarity = $rarity;
		$effect = $effect ?? PvP::getInstance()->getEnchantments()->getEffects()->getRandomEffect($this->getRarity());
