...
use pvp\leaderboards\Structure as LB;
use pvp\PvP;

abstract class Leaderboard extends NonErrorPosition{
	...
	public function __construct(public int $size = 10){
		...
		parent::__construct($arr[0] ?? 0, $arr[1] ?? 0, $arr[2] ?? 0, Server::getInstance()->getWorldManager()->getWorldByName($arr[3] ?? "spawn"));
	...
	public function inLeft(Player $player) : bool{ //HACK!
		return isset(PvP::getInstance()->getLeaderboards()->left[$player->getName()]);
	}
